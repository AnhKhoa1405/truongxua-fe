...
function EventDetail() {
  ...
  const [memberJoinEvent, setMemberJoinEvent] = useState([]);
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        for (let i = 0; i < response.data.length; i++) {
          ...
          const followedSwapp = await getFollowedByIdSwap(
            response.data[i].alumniId
          );
          if (followed == 0 && followedSwapp == 0) {
            alumni.followUser = 3;
          } else {
            alumni.followUser = followed;
          }
          member.push(alumni);
        }
        setMemberJoinEvent(member);
      }
    } catch (err) {
      console.error(err);
    }
  ...
  const renderMemberJoinEvent = () => {
    return memberJoinEvent.map((element, index) => {
      ...
      return (
        <div key={index} className="col-lg-3 col-md-3 col-sm-6">
          <div className="members">
            ...
            <a
              onClick={() => {
                if (element.followUser == 3 && userInfo.Id != element.id) {
            ...
            >
              ...
              {userInfo.Id != element.id
                ? stateFollow[element.followUser].content
                : ""}
            </a>
          </div>
        </div>
  ...
  const connectFollow = async (idAlum) => {
    if (idAlum != userInfo.Id) {
      try {
        ...
        if (response.status === 200) {
          await getMemberInEvent(eventDetail.id);
        }
      } catch (err) {
        console.error(err);
      }
                    ...
                    <div className="main-wraper">
                      ...
                      <div className="event-schedule">
                        ...
                        <h5>
                          ...
                          Bắt đầu vào :{eventDetail.startDate}
                        </h5>
                        <h5>
                          <i className>
                            <svg
                          ...
                          Kết thúc lúc :{eventDetail.endDate}
                        </h5>
                        <a href="#" title>
                          Edit
                        </a>
                      ...
                      <div className="event-loc">
                        ...
                        <h6 style={{ marginTop: 20, fontSize: 22 }}>
                          Các hoạt động có trong sự kiện{" "}
                        </h6>
                        {activity.map((item) => {
                          return <p key={item.id}>{item.name}</p>;
                        })}
                        <strong>
                          Có {memberJoinEvent.length} đã tham gia sự kiện này
                        </strong>
                        <div className="row col-xs-6 merged-10">
                          {renderMemberJoinEvent()}
                        </div>
                        <a href="#" title className="main-btn">
                          Join Now <i className="icofont-arrow-right" />
                        </a>
                      </div>
                    </div>
                  </div>
